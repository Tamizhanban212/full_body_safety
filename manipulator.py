...
class Manipulator3R:
    ...
    def angle(self, a1, a2, a3, duration=1.0):
        self.target_angles = [a1, a2, a3]
        self._animate_to_angles(duration)

    def pos(self, x, y, duration=1.0):
        l1, l2, l3 = self.link_lengths
        phi = 0
        wx = x - l3 * np.cos(np.deg2rad(phi))
        wy = y - l3 * np.sin(np.deg2rad(phi))
    ...
    def home(self, duration=1.0):
        self.angle(0, 0, 0, duration)

    def _animate_to_angles(self, duration, block=True):
        start = np.array(self.angles)
        end = np.array(self.target_angles)
        steps = max(2, int(self.frame_rate * duration))
        angles_seq = np.linspace(start, end, steps)
        self._generate_csv(angles_seq, duration)
        def update(frame):
            self.angles = angles_seq[frame]
            x, y = self._forward_kinematics(self.angles)
            for i in range(3):
        ...
        self.anim = FuncAnimation(self.fig, update, frames=len(angles_seq), interval=interval, blit=True, repeat=False)
        if block:
            self.fig.canvas.draw_idle()
            self.fig.canvas.start_event_loop(duration + 0.5)
...
class ManipulatorApp:
    def __init__(self, root):
        ...
        self.y_entry = ttk.Entry(root, width=8)
        self.y_entry.grid(row=4, column=2)
        ttk.Label(root, text='Duration (s):').grid(row=5, column=1, sticky='e')
        self.duration_entry = ttk.Entry(root, width=8)
        self.duration_entry.grid(row=5, column=2)
        self.duration_entry.insert(0, '1.0')
        self.start_angle_btn = ttk.Button(root, text='Start (Angles)', command=self.start_angle)
        self.start_angle_btn.grid(row=6, column=1, columnspan=2, pady=5)
        self.start_pos_btn = ttk.Button(root, text='Start (X, Y)', command=self.start_pos)
        self.start_pos_btn.grid(row=7, column=1, columnspan=2, pady=5)
        self.home_btn = ttk.Button(root, text='Home', command=self.home)
        self.home_btn.grid(row=8, column=1, columnspan=2, pady=5)

    def start_angle(self, a1=None, a2=None, a3=None, duration=None):
        try:
            if a1 is None:
                a1 = float(self.angle1_entry.get())
            if a2 is None:
            ...
            self.manipulator.angle(a1, a2, a3, duration)
            self.canvas.draw()
        except Exception as e:
            messagebox.showerror('Error', f'Invalid input: {e}')

    def start_pos(self, x=None, y=None, duration=None):
        try:
            if x is None:
                x = float(self.x_entry.get())
            if y is None:
            ...
            if duration is None:
                duration = float(self.duration_entry.get())
            self.manipulator.pos(x, y, duration)
            self.canvas.draw()
        except Exception as e:
            messagebox.showerror('Error', f'Invalid input: {e}')
    ...
    def home(self, duration=None):
        try:
            if duration is None:
                duration = float(self.duration_entry.get())
            self.manipulator.home(duration)
            self.canvas.draw()
        except Exception as e:
            messagebox.showerror('Error', f'Invalid input: {e}')